...
namespace VirtualRaceConsola
{
    public class Carrera
    {
        ...
        public void CorrerCarrera(Tiempo tiempo)
        {
            ...
            for (contadorMinutos = 0; contadorMinutos < (int)tiempo; contadorMinutos++)
            {
                foreach (Auto auto in this.listaDeAuto)
                {
                    auto.Agregar(tiempo);
                }
            }

            for (int elemento = 0; elemento < this.listaDeAuto.Count; elemento++)
            {
                if (elemento == 0)
                {
                    menor = this.listaDeAuto[elemento];
                    mayor = this.listaDeAuto[elemento];
                    continue;
                }

                if ((int)this.listaDeAuto[elemento].ObtenerKilometros() < (int)menor.ObtenerKilometros())
                    menor = listaDeAuto[elemento];
                if ((int)this.listaDeAuto[elemento].ObtenerKilometros() > (int)mayor.ObtenerKilometros())
                    mayor = listaDeAuto[elemento];
            }
            /*menor = mayor = auto1;
            ...
            this.auto5.VolverACero();
            this.auto6.VolverACero();*/
            foreach (Auto objauto in this.listaDeAuto)
            {
                objauto.VolverACero();
            }
        }
        ...
        public void CorrerCarrera(Kilometro kilometro)
        {
            ...
            {           
                foreach (Auto objAuto in this.listaDeAuto)
	            {
		            objAuto.AgregarKilometros((Kilometro)random.Next(0,10));
	            }
            }      
            /*for (contadorKilometros = 0; contadorKilometros < (int)kilometro; contadorKilometros++)
            {
        ...
        public string MostrarCarrera()
        {
            ...
            foreach (Auto objeto in this.listaDeAuto)
            {
                sb.AppendLine(objeto.devolverString());
            }
        ...
        public Carrera()
        {
            ...
            this.auto6 = new Auto();
            random = new Random();*/
            this.listaDeAuto = new List<Auto>();
        }
